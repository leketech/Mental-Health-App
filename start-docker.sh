...
# Simple UnwindMind Docker Startup Script
...
# Navigate to the mentalhealthwebapp directory
cd "$(dirname "$0")/mentalhealthwebapp"

print_status "🚀 Starting UnwindMind application..."
...
while ! curl -f http://localhost:3000 >/dev/null 2>&1; do
    ...
    if [ $timeout -le 0 ]; then
        ...
        # Frontend might still be building, so don't exit
        break
    fi
    echo -n "."
done
